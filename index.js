...
const healthServer = http.createServer((req, res) => {
    ...
    // Default: JSON health response
    const uptime = process.uptime();
    const mem = process.memoryUsage();
    const queueStats = typeof _msgQueue !== 'undefined' && _msgQueue ? _msgQueue.stats : null;
...
const dataDefaults = {
    ...
    'sudo.json': '[]',
    'premium.json': '[]',
    'owner.json': '{"ownerNumber":"","ownerName":"Courtney"}',
    'warnings.json': '{}',
    'prefix.json': '{"prefix":"."}',
...
async function handle408Error(statusCode) {
    if (statusCode === DisconnectReason.connectionTimeout || statusCode === DisconnectReason.timedOut) {
        ...
        // Exponential backoff: 15s → 30s → 60s → 120s, capped at 120s, with ±5s jitter
        const baseDelay = Math.min(15000 * Math.pow(1.5, global.reconnectAttempts - 1), 120000);
...
async function startXeonBotInc() {
    ...
    XeonBotInc.sendMessage = async (jid, content, options) => {
        ...
        if (isGroupJid) {
            ...
            // Add a short delay after pre-warming the cache to let the WhatsApp session
            // settle and avoid hitting rate limits on rapid group sends.
    ...
    setInterval(() => {
        ...
        const gcThreshold = _ramMB <= 300 ? 120 : _ramMB < 512 ? 200 : 450;
        const killThreshold = _ramMB <= 300 ? 150 : _ramMB < 512 ? 230 : 480;
    ...
    const STALE_THRESHOLD_MS = 45 * 60 * 1000; // 45 minutes with no messages = stale
    ...
    try { require('./lib/autocleanup').startAutoCleanup(); } catch (_) {}
...
async function gracefulShutdown(signal) {
    ...
    log('[TRUTH-MD] Shutdown complete — goodbye', 'green');
    process.exit(0);
}
